...
TEST(AppSuiteException, ThrowException)
{
    ...
    EXPECT_THROW(App::IPAddressValidationCheck("10.0.0.0"), std::logic_error);
    std::cout << "EXPECT_THROW 2 - DONE" << std::endl;

    EXPECT_THROW(App::IPAddressValidationCheck("100.64.0.0"), std::logic_error);
    std::cout << "EXPECT_THROW 3 - DONE" << std::endl;

    EXPECT_THROW(App::IPAddressValidationCheck("172.16.0.0"), std::logic_error);
    std::cout << "EXPECT_THROW 4 - DONE" << std::endl;

    EXPECT_THROW(App::IPAddressValidationCheck("192.168.0.0"), std::logic_error);
